...
# IP address of the Android phone's camera (from IP Webcam app)
ANDROID_CAMERA_URL = "http://172.16.21.149:8080/video"  # Replace with your phone's IP and port
...
def track_objects(camera_url=ANDROID_CAMERA_URL):
    global color_picker_state, color_1_hsv_min, color_1_hsv_max, color_2_hsv_min, color_2_hsv_max

    cap = cv2.VideoCapture(camera_url)  # Use the IP stream URL here
    if not cap.isOpened():
...
if __name__ == "__main__":
    track_objects(camera_url=ANDROID_CAMERA_URL)  # Use the Android IP camera URL
