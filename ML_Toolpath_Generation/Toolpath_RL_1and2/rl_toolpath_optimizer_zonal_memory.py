    ...
    for dx in range(-r, r + 1):
        for dy in range(-r, r + 1):
            ...
            if 0 <= nx < visited.shape[0] and 0 <= ny < visited.shape[1]:
                ...
                mask.append(1)  # Out-of-bounds treated as visited
    return tuple(mask)
...
def plot_agent_path_with_heatmap(mask, visit_counts, path, zone_size, output_path):
    ...
    if path:
        ...
        ax.plot(py[0] + 0.5, px[0] + 0.5, 'go')   # Start
        ax.plot(py[-1] + 0.5, px[-1] + 0.5, 'ro')  # End
...
save_episode_csv(progress_log, os.path.join(folder, "progress_log.csv"))
